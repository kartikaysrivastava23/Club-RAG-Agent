...
st.title("Club Information RAG Agent (RAG)")
...
if 'store' not in st.session_state or rebuild:
    try:
        with st.spinner("Building index (this calls embeddings API)..."):
...
# --- User Query ---
question = st.text_input("Ask about the club (answers will be from the club document only):")

if st.button("Ask") and question.strip():
    if 'store' not in st.session_state:
        st.error("Index not ready. Click 'Build / Rebuild Index' in the sidebar.")
    else:
        ...
        st.subheader("Retrieved Passages (Reranked)")
        combined_context = ""
        ...
        system_prompt = (
            ...
            "If the information is clearly NOT present in the context, say: "
            "'I don't know based on the provided club data.' "
            "If partial information exists, summarize what IS present. "
